...
def main():
    audio_file = input("Introduce la ruta del archivo MP3: ")
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"BPM fijo: {bpm_fixed}\n")
        f.write(f"Compases estimados: {measures}\n")
        f.write(f"Tipo de compás: {meter}\n")
        f.write(f"Marcas de tiempo de compases:\n")
        for i, time in enumerate(measure_times):
            f.write(f"Compás {i + 1}: {time:.6f}s\n")
